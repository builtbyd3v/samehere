-- RESTORED 2026-07-09. Commit c459ed7 ("revert: back out contribution v2
-- migration...") deleted this file and 20260712120000_get_heatmap_breakdown_
-- sums_duplicates.sql, claiming the DB changes were rolled back separately.
-- They were NOT: prod still runs these functions today, byte-verified
-- against live pg_proc (posts_award_contribution, comments_award_contribution,
-- reposts_award_contribution, profiles_award_contribution, request_follow,
-- accept_follow, revoke_contribution_same_day, posts_revoke_contribution,
-- comments_revoke_contribution, reposts_revoke_contribution,
-- follows_revoke_connection, get_heatmap, qualifying_length,
-- _log_contribution). The migrations ledger lists both as applied, under
-- MCP-assigned versions 20260709222543 (contribution_v2) and 20260709224657
-- (get_heatmap_breakdown_sums_duplicates) — NOT the filename timestamps
-- below (20260712110000 / 20260712120000), because they were originally
-- applied via the Supabase MCP apply_migration tool, which assigns its own
-- version stamp independent of the local filename. Do not delete this file
-- again without actually reverting the functions in prod first. Because the
-- filename version numbers do not match the ledger's applied version numbers,
-- `supabase db push` / `supabase migration list` may treat this as an
-- unapplied migration and attempt to re-run it — every statement in both
-- files is idempotent (IF NOT EXISTS / IF EXISTS / CREATE OR REPLACE), so a
-- re-run should be harmless, but confirm against the ledger before pushing.
--
