...
// Adapted from Interior's Loading Button.
// Source: https://interior.dev/docs/loading-button
  ...
  return (
    <>
      <motion.button
        ...
        whileTap={disabled || pending || reduced ? undefined : { y: 1, scale: 0.99 }}
        transition={CELL}
        onClick={(event) => {
          if (pending) {
        ...
        className={`path-loading-button relative inline-flex h-11 select-none items-center justify-center overflow-hidden rounded-[0.7rem] bg-[var(--ink)] px-5 text-sm font-semibold text-[var(--canvas)] shadow-[inset_0_1px_0_rgba(255,255,255,0.22),0_12px_30px_-18px_var(--accent-blue)] outline-none transition-[filter,box-shadow] duration-200 hover:brightness-110 focus-visible:ring-2 focus-visible:ring-[var(--accent-blue-strong)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface)] disabled:opacity-50 ${className}`}
      >
