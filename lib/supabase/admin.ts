...
// service_role client — bypasses RLS. SIX sanctioned uses, none of which
...
//   4. Admin hard delete (app/(app)/admin/actions.ts deletePost) — removes
...
//   5. Portfolio metrics (app/api/portfolio/metrics/route.ts) — calls
//      record_portfolio_daily_metric_once only. Session hash, owner, project,
//      and viewer are derived server-side; the body never steers a count.
// (jobs-ingest, weekly-prompt cache, and push notification send were removed
// with those features.)
