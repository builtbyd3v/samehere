...
- [Public profile](./public-profile.md) covers a missing username and, when a live project is configured, a published `/profile/<username>` page plus a `stub-share` / `share-last` UI proof of the absolute Share URL (unit-test lock remains `lib/portfolio/share.test.ts`).
